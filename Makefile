...
COMPONENTS     := src/array src/list src/hashmap src/tree/binary-tree src/stack src/string src/version src/iterator src/set src/CompareFunc src/queue src/heap/binary-heap src/priority-queue
TESTCOMPONENTS := src
...
clean-hard: clean   ##  Perform a clean, as well as removing all products and *.d dependency files.
	@printf "%-8s %s\n" "(RM)" "$(RELBIN:$(ROOTDIR)/%=%)" $(QUIETMODE)
	@$(RM) $(RELBIN)
	@printf "%-8s %s\n" "(RM)" "$(DBGBIN:$(ROOTDIR)/%=%)" $(QUIETMODE)
	@$(RM) $(DBGBIN)
	@printf "%-8s %s\n" "(RM)" "$(DBGRBIN:$(ROOTDIR)/%=%)" $(QUIETMODE)
	@$(RM) $(DBGRBIN)
	@printf "%-8s %s\n" "(RM)" "$(TESTBIN:$(ROOTDIR)/%=%)" $(QUIETMODE)
	@$(RM) $(TESTBIN)
